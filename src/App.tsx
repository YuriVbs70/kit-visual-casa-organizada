...
import {
  ...
  Package, Printer, ShieldCheck, ShoppingBag, Smartphone, Sparkles,
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-neutral-900 antialiased">
      ...
      {/* 4 — URGÊNCIA */}
      <section className="bg-[#DFFF04] px-5 py-16 text-center"><Reveal><p className="text-xs font-black uppercase tracking-[.2em]">Você não precisa esperar a próxima faxina</p><h2 className="mx-auto mt-3 max-w-4xl text-3xl font-black leading-tight md:text-5xl">Comece pelo ambiente que mais pesa na sua rotina. Um guia, uma decisão, um passo de cada vez.</h2><button onClick={openModal} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-neutral-950 px-7 py-4 text-sm font-extrabold uppercase tracking-wide text-white">Quero começar com clareza<ArrowRight className="h-5 w-5" /></button></Reveal></section>

      {/* 5 — IDEAL PARA */}
      <section className="bg-[#EEEEEE] py-20"><div className="mx-auto max-w-6xl px-5"><Reveal className="text-center"><p className="eyebrow">Ideal para você que deseja</p><h2 className="section-title">Mais praticidade e menos peso mental dentro de casa.</h2></Reveal><div className="mt-12 grid gap-4 md:grid-cols-2">{[
        'Saber por onde começar sem tentar organizar a casa inteira de uma vez.', 'Encontrar um lugar lógico para objetos que vivem espalhados.', 'Reduzir o tempo gasto procurando, guardando e reorganizando.', 'Criar uma rotina que outras pessoas da casa consigam entender.', 'Organizar usando o que já possui antes de comprar novos organizadores.', 'Ter instruções rápidas para consultar pelo celular ou imprimir.',
      ].map((item, i) => <Reveal key={item} delay={(i % 2) * 80}><div className="flex h-full gap-4 rounded-xl bg-white p-5 shadow-sm"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-neutral-900 text-[#DFFF04]"><Check className="h-5 w-5" /></div><p className="font-semibold leading-relaxed text-neutral-700">{item}</p></div></Reveal>)}</div></div></section>
